...
export async function fetchDriverContext() {
  ...
  try {
    ...
    return {
      data: null,
      error: { message: fallbackResult.error?.message ?? 'Connessione al server Vyko non riuscita.' },
    }
  }
}
...
export async function createCompanyDriverAccount({ companyId, driver, password }) {
  ...
  try {
    ...
    return {
      data: null,
      error: { message: 'Server Vyko non raggiungibile. Controlla il deploy Netlify.' },
    }
  }
}
