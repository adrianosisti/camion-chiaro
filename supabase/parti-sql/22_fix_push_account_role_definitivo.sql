-- FIX DEFINITIVO NOTIFICHE PUSH - ACCOUNT ROLE
-- Esegui questo file se "Verifica notifiche" dice:
-- new row for relation "push_subscriptions" violates check constraint "push_subscriptions_account_role_check"
...
begin
  for current_constraint in
    ...
    where conrelid = 'public.push_subscriptions'::regclass
      and contype = 'c'
      and (
        conname = 'push_subscriptions_account_role_check'
        or pg_get_constraintdef(oid) ilike '%account_role%'
      )
  loop
    execute format('alter table public.push_subscriptions drop constraint if exists %I', current_constraint.conname);
  end loop;
end;
$$;
...
set account_role = case
  when lower(trim(account_role)) in ('autista', 'driver') then 'driver'
  when lower(trim(account_role)) in ('azienda', 'company') then 'company'
  else 'driver'
end;
...
begin
  ...
  current_account_role = case
    when current_driver_id is null then 'company'
    else 'driver'
  end;
...
select 'Notifiche push sistemate: account_role accetta company/driver.' as risultato;
